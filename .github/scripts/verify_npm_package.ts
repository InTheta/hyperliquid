...
try {
  ...
  await Deno.writeTextFile(
    ...
    [
      ...
      `import * as explorer from ${JSON.stringify(`${packageJson.name}/api/explorer`)};`,
      ...
      "for (const [name, mod] of Object.entries({ sdk, utils, info, exchange, explorer, subscription, signing })) {",
