...
const _l1_js_1 = require("./_l1.js");
const _userSigned_js_1 = require("./_userSigned.js");
...
async function signMultiSigOuter(args) {
    ...
    const multiSigActionHash = (0, _l1_js_1.createL1ActionHash)({
...
async function signMultiSigL1(args) {
    ...
    const outerSigner = await (0, _abstractWallet_js_1.getWalletAddress)(args.signers[0]);
    // --- Collect inner signatures from all signers -----------
    const innerSignatures = await Promise.all(args.signers.map((signer) => (0, _l1_js_1.signL1Inner)({
        signer,
        action: args.action,
...
async function signMultiSigUserSigned(args) {
    ...
    const outerSigner = await (0, _abstractWallet_js_1.getWalletAddress)(args.signers[0]);
    // --- Collect inner signatures from all signers -----------
    const innerSignatures = await Promise.all(args.signers.map((signer) => (0, _userSigned_js_1.signUserSignedInner)({
        signer,
        action: args.action,
        types: args.types,
        multiSigUser: args.multiSigUser,
