...
class ReconnectingWebSocket extends EventTarget {
    ...
    _awaitSocketLifecycle(socket) {
        return new Promise((resolve) => {
            ...
            const settle = (init) => {
                if (this._settleLifecycle !== settle)
            ...
            const onError = (event) => {
                ...
                const errorEvent = message !== undefined || error !== undefined
                    ? new event.constructor("error", { message, error })
                    : new Event("error");
    ...
    get url() {
        ...
        if (typeof this._urlProvider === "function")
            return "";
        try {
    ...
    get bufferedAmount() {
        ...
        for (const data of this._messageBuffer) {
            if (typeof data === "string")
                total += new TextEncoder().encode(data).length;
     ...
     */
    get extensions() {
        if (this.readyState === ReconnectingWebSocket.CONNECTING)
            return "";
    ...
    _setAttributeListener(type, handler) {
