...
Object.defineProperty(exports, "WebSocketRequestError", { enumerable: true, get: function () { return _dispatcher_js_1.WebSocketRequestError; } });
const _events_js_1 = require("./_events.js");
const _keepAlive_js_1 = require("./_keepAlive.js");
...
class WebSocketTransport {
    ...
    _hlEvents;
    _dispatcher;
    _keepAlive; // self-contained
    _subscriptionManager;
    /** Creates the transport and immediately starts connecting. */
    constructor(options) {
        this.isTestnet = options?.isTestnet ?? false;
        this.socket = new mod_js_1.ReconnectingWebSocket(options?.url ?? (this.isTestnet ? exports.TESTNET_API_WS_URL : exports.MAINNET_API_WS_URL), options?.reconnect);
        this._hlEvents = new _events_js_1.HyperliquidEventTarget(this.socket);
        this._dispatcher = new _dispatcher_js_1.WebSocketDispatcher(this.socket, this._hlEvents, options?.timeout === undefined ? 10_000 : options.timeout);
        this._keepAlive = new _keepAlive_js_1.WebSocketKeepAlive(this.socket, this._hlEvents, options?.keepAlive);
        this._subscriptionManager = new _subscriptionManager_js_1.WebSocketSubscriptionManager(this.socket, this._dispatcher, this._hlEvents, options?.resubscribe ?? true);
    }
    /**
    ...
    async request(endpoint, payload, signal) {
        ...
        return await this._dispatcher.request("post", wrapped, signal);
    }
    /**
     ...
     * ```
     */
    subscribe(channel, payload, listener, options) {
    ...
    ready(signal) {
        return new Promise((resolve, reject) => {
            const failTerminated = () => reject(new _dispatcher_js_1.WebSocketRequestError("Failed to establish WebSocket connection", {
                cause: this.socket.terminationSignal.reason,
            }));
            const failAborted = () => reject(new _dispatcher_js_1.WebSocketRequestError("Waiting for the connection was aborted", { cause: signal?.reason }));
            if (signal?.aborted)
                return failAborted();
            if (this.socket.terminationSignal.aborted)
                return failTerminated();
            if (this.socket.readyState === mod_js_1.ReconnectingWebSocket.OPEN)
                return resolve();
