...
exports.WebSocketDispatcher = exports.WebSocketRequestError = void 0;
...
class WebSocketDispatcher {
    ...
    constructor(socket, hlEvents, timeout) {
        ...
        const handleDisconnect = () => {
            this._queue.forEach(({ sent, payload, reject }) => reject(new WebSocketRequestError(sent ? "WebSocket connection closed" : "WebSocket connection closed before the request was sent", { request: payload })));
            this._queue = [];
        };
    ...
    async request(method, payload, signal) {
        ...
        try {
            ...
            const id = "id" in request ? request.id : (0, _id_js_1.requestToId)(request);
            // --- Send or queue -----------------------------------------------------
            const frame = JSON.stringify(request);
            const sent = this._socket.readyState === mod_js_1.ReconnectingWebSocket.OPEN;
            if (sent)
    ...
    _findByEcho(echo) {
        ...
        for (const pending of this._queue) {
            ...
            if (!(0, _id_js_1.isSubset)(payload, echo))
                continue;
            const score = (0, _id_js_1.specificity)(payload);
...
//# sourceMappingURL=_dispatcher.js.map