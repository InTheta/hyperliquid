...
Deno.test("WebSocketSubscriptionManager", async (t) => {
  ...
  await t.step("autoResubscribe", async (t) => {
    ...
    await t.step("onError: terminal loss notifies every listener once with the reason, isolating throws", async () => {
      ...
      socket.mockMessage(RESPONSES.subscriptionResponse("subscribe", payload));
      const firstSubscription = await p1;
      const secondSubscription = await manager.subscribe("test", payload, () => {}, { onError: (e) => seen.push(e) });

      assertFalse(firstSubscription.failureSignal.aborted);
      assertFalse(secondSubscription.failureSignal.aborted);
      ...
      assert(firstSubscription.failureSignal.aborted);
      assert(secondSubscription.failureSignal.aborted);
      assert(firstSubscription.failureSignal.reason instanceof WebSocketRequestError);
      assertEquals(manager._subscriptions.size, 0);
    });
