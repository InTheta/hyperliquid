...
import { allMids as fetchAllMids, fundingHistory, metaAndAssetCtxs, outcomeMeta, perpDexs, predictedFundings, spotMetaAndAssetCtxs, } from "../api/info/mod.js";
import { allDexsAssetCtxs, allDexsClearinghouseState, allMids as subscribeAllMids, } from "../api/subscription/mod.js";
import { formatPrice, formatSize } from "./_format.js";
...
export class HyperliquidMarketCache {
    ...
    async startAllMids(dex) {
        ...
        const subscription = await subscribeAllMids({ transport: this.subscriptionTransport }, params, (event) => this.applyAllMids(event, key));
    ...
    async startAllDexsAssetCtxs() {
        ...
        this._allDexsAssetCtxsSubscription = await allDexsAssetCtxs({ transport: this.subscriptionTransport }, (event) => this.applyAllDexsAssetCtxs(event));
        return this._allDexsAssetCtxsSubscription;
    }
    ...
    async startAllDexsClearinghouseState(user) {
        ...
        if (existing)
            return existing;
        const subscription = await allDexsClearinghouseState({ transport: this.subscriptionTransport }, { user }, (event) => this.applyAllDexsClearinghouseState(event));
    ...
    async _refreshMetadataAndContexts(updateMetadata, updateContexts) {
        ...
        const [spotTuple, ...perpTuples] = await Promise.all([
            ...
            ...dexEntries.map((entry) => metaAndAssetCtxs(config, dexRequestName(entry.name) ? { dex: entry.name } : undefined)),
        ]);
        const rawByDex = {};
    ...
    async _refreshAllMids() {
        const config = { transport: this.transport };
        const dexEntries = await this._loadDexEntries();
        const results = await Promise.all(dexEntries.map((entry) => fetchAllMids(config, dexRequestName(entry.name) ? { dex: entry.name } : undefined)));
        dexEntries.forEach((entry, index) => {
