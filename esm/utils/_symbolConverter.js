import { meta, outcomeMeta, perpDexs, spotMeta, } from "../api/info/mod.js";
/**
...
export class SymbolConverter {
    ...
    // Outcome markets
    // ============================================================
    _processOutcomeMarkets(outcomeMetaData) {
        ...
        outcomeMetaData.outcomes.forEach((outcome) => {
            if (fallbackOutcomes.has(outcome.outcome))
                return;
     ...
     */
    _outcomeSlug(outcome, side, question) {
     ...
     */
    _recurringPriceSlug(spec, side, bucketIndex) {
        const fields = {};
        ...
        if (!assetClass || !underlying || !expiry)
            return null;
