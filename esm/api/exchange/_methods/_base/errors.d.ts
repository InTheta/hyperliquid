...
export declare function isErrorResponse(r: unknown): r is TopLevelError | BulkError | SingleError;
/**
...
type ExcludeTopLevelError<T> = T extends {
    status: "err";
} ? never : T;
/** Filter out error variants from `response.data.statuses[]` array. */
type ExcludeBulkError<T> = T extends {
    response: {
        data: {
...
type ExcludeSingleError<T> = T extends {
    response: {
        data: {
            status: infer S;
        };
