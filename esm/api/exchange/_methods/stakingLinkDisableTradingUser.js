...
const StakingLinkDisableTradingUserActionSchema = /* @__PURE__ */ (() => {
    return v.omit(v.object(StakingLinkDisableTradingUserRequest.entries.action.entries), ["signatureChainId", "hyperliquidChain", "nonce"]);
})();
 ...
 * @see https://hyperliquid.gitbook.io/hyperliquid-docs/trading/fees#staking-linking
 */
export function stakingLinkDisableTradingUser(config, params, opts) {
    const action = canonicalize(StakingLinkDisableTradingUserActionSchema, parse(StakingLinkDisableTradingUserActionSchema, { type: "stakingLinkDisableTradingUser", ...params }));
