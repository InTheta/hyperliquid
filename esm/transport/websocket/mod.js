 ...
 * @module
 */
import { ReconnectingWebSocket } from "../../deps/jsr.io/@nktkas/rews/4.1.0/mod.js";
import { WebSocketDispatcher, WebSocketRequestError } from "./_dispatcher.js";
import { HyperliquidEventTarget } from "./_events.js";
import { WebSocketKeepAlive } from "./_keepAlive.js";
...
export class WebSocketTransport {
    ...
    constructor(options) {
        ...
        this._dispatcher = new WebSocketDispatcher(this.socket, this._hlEvents, options?.timeout === undefined ? 10_000 : options.timeout);
        this._keepAlive = new WebSocketKeepAlive(this.socket, this._hlEvents, options?.keepAlive);
        this._subscriptionManager = new WebSocketSubscriptionManager(this.socket, this._dispatcher, this._hlEvents, options?.resubscribe ?? true);
    }
    /**
    ...
    ready(signal) {
        return new Promise((resolve, reject) => {
            const failTerminated = () => reject(new WebSocketRequestError("Failed to establish WebSocket connection", {
                cause: this.socket.terminationSignal.reason,
            }));
            const failAborted = () => reject(new WebSocketRequestError("Waiting for the connection was aborted", { cause: signal?.reason }));
            if (signal?.aborted)
                return failAborted();
